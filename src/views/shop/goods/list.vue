			...
			<el-tab-pane :label="tab.name" :key="tabI" v-for="(tab,tabI) in tabbars">
				...
				<button-search ref="buttonSearch" placeholder="要搜索的商品名称" @search="searchEvent">
					...
					<template #left>
						...
						<el-button size="mini" @click="back">还原</el-button>
					</template>
					<!-- 高级搜索表单 -->
					<template #form>
				...
				<el-table border class="mt-3" :data="glist" style="width: 100%" @selection-change="handleSelectionChange">
					...
					<el-table-column label="商品" width="250">
						<template slot-scope="scope">
							<div class="media">
								<img class="mr-3" style="width: 60px;height: 60px;" :src="scope.row.picUrl">
								<div class="media-body">
									<p class="mt-0">{{scope.row.name}}</p>
					...
					<el-table-column label="商品简介" width="250">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.description}}</p>
								</div>
							</div>
						</template>
					</el-table-column>
					...
					<el-table-column label="创建时间" width="200">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.addTime}} </p>
								</div>
							</div>
						</template>
					</el-table-column>
					<el-table-column prop="status" align="center" label="商品状态">
						<template slot-scope="scope">
							<el-button type="success" size="mini" @click="scope.row.ischeck = 1" plain>审核通过
							</el-button>
					...
					<el-table-column label="价格" width="100">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.price}} 元</p>
								</div>
							</div>
						</template>
					</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			back(){
				this.axios.get('/admin/product/list', {
					params: {
						pageNum: 1,
						pageSize: 100
					},
			...
			deleteItem(item) {
				...
				this.$confirm('此操作将永久删除该商品, 是否继续?', '提示', {
					...
					this.axios.get('/admin/product/delete/' + item.row.id, {
						headers: {
							'Authorization': window.sessionStorage.getItem('token')
						}
					}).then(res => {
