		...
		<el-tabs v-model="tabIndex" @tab-click="handleClick">
			<el-tab-pane :label="tab.name" :key="tabI" v-for="(tab,tabI) in tabbars">
				...
				<!--
				 ...
				 价格(元)（1000.00）
				 -->
				<el-table border class="mt-3" :data="glist" style="width: 100%" @selection-change="handleSelectionChange">
					...
					<el-table-column label="商品" width="380">
						<template slot-scope="scope">
							<div class="media">
								<img class="mr-3" style="width: 60px;height: 60px;" src="https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=1116745294,1146941663&fm=26&gp=0.jpg">
								<div class="media-body">
									<p class="mt-0">{{scope.row.order.creatTime}}</p>
									<p class="mb-0">分类：{{scope.row.categoryName}}</p>
									<!-- <p class="mb-0">时间：{{scope.row.addTime}}</p> -->
								</div>
							</div>
						</template>
					</el-table-column>
					<!-- <el-table-column prop="glist.id" align="center" label="商品类型">
					</el-table-column> -->
					<el-table-column label="订单号" width="300">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.order.id}}</p>
								</div>
							</div>
						</template>
					</el-table-column>
					...
					<!-- <el-table-column label="总库存" width="200">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.stock}}</p>
								</div>
							</div>
						</template>
					</el-table-column> -->
					...
					<el-table-column label="创建时间" width="300">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.order.creatTime}} </p>
								</div>
							</div>
						</template>
					</el-table-column>
					<el-table-column prop="status" align="center" label="订单状态">
						<template slot-scope="scope">
							<el-button type="success" size="mini" @click="scope.row.ischeck = 1" plain>订单完成
							</el-button>

							<!-- <el-button class="ml-0 mt-2" type="danger" size="mini" @click="scope.row.ischeck = 2" plain>审核通过
							</el-button> -->
					...
					<!-- <el-table-column label="实际销量" width="200">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.sale}} 件</p>
								</div>
							</div>
						</template>
					</el-table-column> -->
					...
					<el-table-column label="价格" width="200">
						<template slot-scope="scope">
							<div class="media">
								<div class="media-body">
									<p class="mb-0">{{scope.row.order.prices}} 元</p>
								</div>
							</div>
						</template>
					</el-table-column>
					<el-table-column align="center" label="操作" width="150">
						<template slot-scope="scope">
							<el-button-group>
								...
								<el-button type="danger" size="mini" plain @click="deleteItem(scope.row.order.id)">删除</el-button>
							</el-button-group>
						</template>
					</el-table-column>
				</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			// 生成数据
			getlist() {
				this.axios.get('/admin/order/getOrders', {
					headers: {
			...
			deleteItem(data) {
				...
				this.$confirm('此操作将永久删除该订单, 是否继续?', '提示', {
					...
					// this.axios.delete('/admin/order/deleteOrderById?id='+this.oid, {
					// 	// params: {
					// 	// 	id:data
					// 	// },
					// 	headers: {
